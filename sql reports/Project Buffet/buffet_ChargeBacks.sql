select distinct
    wo.scode 'WO#',p.scode Property, u.sstatus 'Current Unit Status',
...
(SELECT TOP 1 CONVERT(date, start) 
   ...
   ORDER  BY start DESC) AS 'Rent Ready Date',
    ...
    wo.SREQUESTEDBY 'Created User', wo.DTDATEIN 'Created date', wo.SUPDATEDBY 'Updated User', wo.DTUPDATEDT 'Updated Date',
    au.uName  'Approved By', wfs.dtComplete 'Date Approved', wo.dtinvoice, 
    wo.ctotal0 'WO Total',
    
(select sum(stotalamount) from yardi_stage.dbo.trans where hparent2=wo.hchgrcd and wo.htenant=hperson and hchgrcd!=0 and hchgrcd is not null)'Amount Charged Back',
(select sum(samountpaid) from yardi_stage.dbo.trans where hparent2=wo.hchgrcd and wo.htenant=hperson and hchgrcd!=0 and hchgrcd is not null)'Amount paid on Charge Back'
