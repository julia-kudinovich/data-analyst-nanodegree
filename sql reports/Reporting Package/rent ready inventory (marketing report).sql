...
(SELECT Count(DISTINCT mobilephone) 
   ...
   WHERE  primaryproperty = p.scode  AND createdon >= 
																	(SELECT TOP 1 CONVERT(date, start) 
																		   FROM   [crm].[dbo].[constructionhistory] 
																		   WHERE  p.scode LIKE LEFT(NAME, 7) AND constructionstatus = 'Rent Ready' AND mobilephone IS NOT NULL
																		   ORDER  BY start DESC)) 
...
(SELECT Count(DISTINCT lastname) 
   FROM   [crm].[dbo]. [leads] 
	WHERE  primaryproperty = p.scode AND createdon >= 
																	(SELECT TOP 1 CONVERT(date, start) 
																		   FROM   [crm].[dbo].[constructionhistory] 
																		   WHERE  p.scode LIKE LEFT(NAME, 7) AND constructionstatus = 'Rent Ready' AND mobilephone IS NULL
																		   ORDER  BY start DESC)) as 'Leads', 
...
LEFT JOIN tenant t ON t.hproperty = u.hproperty AND t.istatus NOT IN ( 5, 6, 7, 8, 9 ) 
